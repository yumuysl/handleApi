...
function sign(
  ...
  const handleHeaders = (headers: { [key: string]: any }) => {
    let keyArr: string[] = Object.keys(headers)
      .map((item) => item.toLowerCase())
      .sort();
    return keyArr.filter(
      (key) =>
        key.includes("host") ||
        key.includes("x-acs") ||
  ...
  const HashedCanonicalRequest: string = encodeURIComponent(
    crypto.createHash("sha256").update(CanonicalRequest).digest("base64")
  ); //请求体的hash值后再Base64编码后的字符串
...
function request(
  ...
  //请求头键名，除Authorization
  const SignedHeaders = (headers: { [key: string]: string }) => {
    let keys = Object.keys(headers)
    return keys.join("&");
  };
  //签名
  const Signature = sign(method, path, param, headers, access_key_secret);
...
request("post", adress, request_path, param, access_key_id, access_secret)
  .then((res) => {
